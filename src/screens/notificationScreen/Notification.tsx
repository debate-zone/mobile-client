...
export const NotificationScreen = ({navigation}: RootScreenProps) => {
    ...
    const navigateOnPress = async (outputNotificationItem: OutputNotification) => {
    ...
    const navigateDebateOnPress = async (outputNotificationItem: OutputNotification) => {
        markAsReadNotification(outputNotificationItem._id)
        navigation.navigate('JoinDetailsScreen', {id: outputNotificationItem.data.debateZoneId})
    ...
    const markAsReadNotification = async (id: string) => {
        try {
            await request<OutputNotification>(
                'PUT',
                '/notification/v1/notifications/read', {id}
            )
    ...
    return <View style={style.container}>
        {notificationList?.notifications && notificationList?.notifications.length > 0 ? <TouchableOpacity style={{alignItems: "flex-end"}} onPress={markAllAsRead}>
            <Text style={style.markAllRead}>Mark all as read</Text>
        </TouchableOpacity> : <></> }

        <NotificationList onPress={navigateOnPress} markAsReadOnPress={markAsReadNotification} outputNotificationList={notificationList}/>
    </View>
}
